...
describe("production recording gates", () => {
  ...
  it("requires a bounded cron secret for external workers", () => {
    expect(clinicalAudioPreflight({ ...env, CLINICAL_AUDIO_WORKER_MODE: "external", CLINICAL_AUDIO_CRON_SECRET: "short" }).checks.find(c => c.name === "HTTP cron secret")?.result).toBe("FAIL");
    expect(clinicalAudioPreflight({ ...env, CLINICAL_AUDIO_WORKER_MODE: "external" }).checks.find(c => c.name === "HTTP cron secret")?.result).toBe("PASS");
  });
