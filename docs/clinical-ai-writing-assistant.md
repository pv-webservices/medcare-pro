...
Chief complaint and HPI retain MEDIUM risk: reviewed spellings (including `diabates → diabetes`), narrowly contextual same-tense agreement for patient/he/she, article edits and safe formatting. The other six fields retain VERY_HIGH risk: the smaller reviewed spelling set and existing patient agreement/article rules; the narrative diabetes correction and pronoun agreement are withheld. Arbitrary one-character substitutions are rejected, including `had → has`, `cold → cool`, and unknown medication spellings. Medication-like text freezes lexical edits. `sever → severe` requires an explicit symptom context. Only the initial letter of sentence-initial patient/the/he/she may change case; internal case and units such as `nM`, `μM`, `ms` and `mmol` remain exact.
