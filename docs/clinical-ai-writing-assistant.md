...
MedCare computes its own grapheme-aware enclosing changed span and compares the complete texts using ordered Unicode tokens. Provider `changed`, confidence, categories, fragments, reasons and `suggestions[]` never authorize or veto a candidate. The UI response is constructed locally from the validated text and local diff; its legacy `confidence: HIGH` is compatibility metadata, not a model assessment. Unchanged text returns `UNCHANGED`, independently of provider flags. Whitespace-only candidates, and punctuation-only candidates in spelling mode, return `UNCHANGED`. When the complete candidate is unsafe, MedCare aligns its words with the original and keeps only one-for-one word substitutions that each pass the same validator (greedily, in reading order, under the same edit cap), applied to the doctor's original text; insertions, deletions and reorderings are never kept. A non-empty result returns `PARTIAL`. Otherwise the response is `changed=false`, empty suggested text/fragments and `SAFETY_REJECTED`; unsafe output is never returned or retried automatically. A misspelling may be corrected to a duration unit (yers → years, weks → weeks) only when that unit is the single nearest quantity word; counts and frequencies (two, twice, daily) stay protected, and insertions of words next to a correction do not stop it being kept. In narrative fields only, a misspelling of 7+ letters may take one extra edit (diareaha → diarrhea) when the candidate is in the curated `COMMON_CLINICAL_TERMS` list and no other listed term is as close; high-risk fields keep their single edit. The response lists changes word by word (fiverr → fever), falling back to a whole-word span for punctuation or capitalisation.
...
An `AiRun` reservation is committed before provider contact. A short transaction locks the tenant row and counts indexed rolling usage, enforcing twelve requests/user/minute, twenty-four requests/registration/five minutes and one hundred requests/tenant/hour (`AI_RUN_LIMITS`) across app instances. Failures and safety rejections consume allowance. Provider network waiting occurs outside the transaction. Reservations left STARTED after process failure still count until their window expires; operational review can identify these records.
