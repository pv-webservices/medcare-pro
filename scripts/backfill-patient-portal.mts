/** Patient Portal entitlement/role installation. Dry-run by default.
 * Never creates accounts/links, sends messages, or rewrites custom roles.
 */
import "dotenv/config";
import { prisma } from "@/lib/prisma";
import { DEFAULT_FEATURES, DEFAULT_PLAN_KEY } from "@/lib/defaultFeatures";
import { planPatientPortalRoleMigration } from "@/lib/patientPortalRoleMigration";
...
async function main() {
  console.log(`Patient Portal backfill: ${apply ? "APPLY" : "DRY RUN"}`);
  const feature = DEFAULT_FEATURES.find(
    (entry) => entry.key === "patient_portal",
  )!;
  ...
  const link = existing
    ...
    : null;
  console.log(
    `${existing ? "KEEP" : "WOULD CREATE"} patient_portal CORE feature; ${link ? "KEEP" : "WOULD LINK"} standard plan. Existing switches/overrides remain authoritative.`,
  );
  if (apply)
  ...
  for (const role of roles) {
    const proposal = planPatientPortalRoleMigration(role);
    console.log(
    ...
    if (apply && proposal.status === "ELIGIBLE")
      await prisma.$transaction(async (tx) => {
        ...
        const checked = planPatientPortalRoleMigration(fresh);
...
main()
  .catch(() => {
    console.error(
      "Patient Portal backfill failed. Check schema/configuration; no clinical content is logged.",
    );
    process.exitCode = 1;
  })
