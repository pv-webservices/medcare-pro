...
type Run = { id: string; status: string; failureCode: string | null; transcriptId: string | null };
...
export default function TranscriptPanel({ recordingId, onSeek }: { recordingId: string; onSeek: (milliseconds: number) => Promise<void> }) {
  ...
  return <section aria-label="Consultation transcript" className="mt-4 min-w-0 space-y-4 border-t border-line pt-4">
    <div className="flex flex-wrap items-center justify-between gap-2"><h4 className="font-semibold">Transcript · Saaras v4</h4><span role="status" className="text-sm text-muted">{run?.status ?? "Not generated"}</span></div>
    {(!run || ["FAILED", "TIMED_OUT", "CANCELLED"].includes(run.status)) && <button disabled={busy} className={button} onClick={() => void save(base, {})}>{run ? "Retry with Sarvam" : "Generate transcript"}</button>}
    ...
    {transcript && <>
      ...
      <ol className="space-y-3" start={segmentPage * 50 + 1}>{transcript.segments.slice(segmentPage * 50, segmentPage * 50 + 50).map((segment) => <li key={segment.id} className="min-w-0 space-y-2 rounded-lg border border-line p-3">
        <button className={button} onClick={() => void onSeek(segment.startMs).catch(() => setError("Audio is unavailable or no longer retained."))}>Listen at {(segment.startMs / 1000).toFixed(1)}s · {transcript.speakers.find((speaker) => speaker.speakerLabel === segment.speakerLabel)?.speakerType ?? "UNKNOWN"}</button>
        <p className="whitespace-pre-wrap break-words">{effectiveText(segment)}</p>
