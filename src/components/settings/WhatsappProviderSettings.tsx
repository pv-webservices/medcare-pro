...
export default function WhatsappProviderSettings({ initialValue, canEdit }: { initialValue: WhatsappConfigurationView; canEdit: boolean }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {qr?.reconnect && <QrModal qr={qr} close={() => { setQr(null); void reload(); }} connected={(message) => { setNotice(message); setQr(null); void reload(); }} />}
...
function QrModal({ qr, close, connected }: { qr: { deviceId: string; value: string }; close: () => void; connected: (message: string) => void }) {
  const [error, setError] = useState<string | null>(null);
  useQrPolling(qr.deviceId, connected, setError);
  return <ModalShell title="Scan this QR using WhatsApp" close={close}>{error && <p role="status" className="text-sm text-warn-ink">{error}</p>}<QrImage value={qr.value} /></ModalShell>;
}
