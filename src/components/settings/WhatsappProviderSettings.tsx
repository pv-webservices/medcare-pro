...
export default function WhatsappProviderSettings({ initialValue, canEdit }: { initialValue: WhatsappConfigurationView; canEdit: boolean }) {
  ...
  async function deviceAction(deviceId: string, action: string) {
    ...
    const payload = await response.json() as { success?: boolean; data?: { webhookUrl?: string | null; message?: string }; error?: string };
  ...
  return (
    <div className="space-y-6">
      ...
      {connectOpen && <ConnectModal accounts={value.accounts.filter((account) => account.enabled && account.devices.length < account.deviceLimit)} qr={qr} setQr={setQr} close={() => { setConnectOpen(false); setQr(null); void reload(); }} connected={(message) => { setNotice(message); setConnectOpen(false); setQr(null); void reload(); }} />}
    </div>
  );
}
...
function DeviceRow({ device, canEdit, primary, backup, act }: { device: WhatsappConfigurationView["accounts"][number]["devices"][number]; canEdit: boolean; primary: boolean; backup: boolean; act: (action: string) => Promise<void> }) {
  const status: Record<typeof device.connectionStatus, { tone: StatusTone; help: string }> = {
    CONNECTED: { tone: "ok", help: "WhatsApp connection confirmed." },
    DISCONNECTED: { tone: "alert", help: "WhatsApp is disconnected." },
    UNKNOWN: { tone: "neutral", help: "Status has not been confirmed." },
    PENDING: { tone: "warn", help: "Waiting for WhatsApp connection." },
  };
  ...
  return <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-line p-4"><div className="space-y-1"><p className="font-semibold text-ink">{device.name} · +{device.phoneNumber}</p><div className="flex flex-wrap items-center gap-2"><StatusPill tone={state.tone}>{device.connectionStatus}</StatusPill><span className="text-sm text-muted">{primary ? "Organisation primary" : backup ? "Backup" : ""}</span></div><p className="text-xs text-muted">{state.help} Last checked: {checked}.</p><p className="text-xs text-muted">{device.webhookConfigured ? "Webhook configured." : "Webhook not configured."}</p></div>{canEdit && <div className="flex flex-wrap gap-2"><Button size="sm" onClick={() => void act("refresh")}>Refresh status</Button><Button size="sm" onClick={() => { if (confirm("Disconnect this WhatsApp session? The device remains configured and continues to occupy a provider slot.")) void act("disconnect"); }}>Disconnect</Button>{device.webhookConfigured ? <Button size="sm" onClick={() => { if (confirm("Regenerate the webhook URL? The existing URL will immediately stop working.")) void act("regenerateWebhook"); }}>Regenerate webhook URL</Button> : <Button size="sm" onClick={() => void act("setupWebhook")}>Set up webhook</Button>}<Button size="sm" variant="danger" onClick={() => { if (confirm("Permanently remove this device from RkvRobo and MedCarePro? This frees its provider slot and cannot be treated as simple UI cleanup.")) void act("remove"); }}>Remove</Button></div>}</div>;
}
...
function ConnectModal({ accounts, qr, setQr, close, connected }: { accounts: WhatsappConfigurationView["accounts"]; qr: { deviceId: string; value: string } | null; setQr: (value: { deviceId: string; value: string } | null) => void; close: () => void; connected: (message: string) => void }) {
  ...
  async function connect(event: FormEvent) { event.preventDefault(); setBusy(true); setError(null); try { const response = await fetch("/api/settings/whatsapp/devices/connect", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phoneNumber, name, ...(accounts.length > 1 ? { providerAccountId: accountId } : {}) }) }); const payload = await response.json() as { success?: boolean; data?: { deviceId: string; qr: string | null; alreadyConnected: boolean; message: string }; error?: string }; if (!response.ok || !payload.success || !payload.data) throw new Error(payload.error ?? "Could not connect WhatsApp."); if (payload.data.alreadyConnected) { connected(payload.data.message); return; } if (!payload.data.qr) throw new Error("RkvRobo did not return a QR code. Refresh status before retrying."); setQr({ deviceId: payload.data.deviceId, value: payload.data.qr }); } catch (caught) { setError(caught instanceof Error ? caught.message : "Could not connect WhatsApp."); } finally { setBusy(false); } }
