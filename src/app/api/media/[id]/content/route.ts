...
async function handleContentRequest(
  ...
  // Support Range requests (206 Partial Content) for streaming media like MP4 video
  if (rangeHeader) {
...
export async function GET(request: Request, props: RouteProps) {
  return handleContentRequest(request, props, false);
}

export async function HEAD(request: Request, props: RouteProps) {
  return handleContentRequest(request, props, true);
}
