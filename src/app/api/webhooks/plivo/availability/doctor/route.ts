...
import { handleDoctorMenuInput } from "@/lib/telephony/availability";
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    return xmlResponse(
      await handleDoctorMenuInput({ requestUrl: request.url, clinic, digits }),
    );
  } catch {
    console.error("Could not process the Plivo doctor menu callback.");
    return new Response("Service unavailable.", { status: 503 });
  }
}
